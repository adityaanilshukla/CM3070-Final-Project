...
def evaluate_fsm_stability_response_time_smoothness_and_time(num_episodes=100):
    ...
    min_gimbal_smoothness = []  # List to store the minimum gimbal changes for each episode
    ...
    min_throttle_smoothness = []  # List to store the minimum throttle changes for each episode
    ...
    for episode in range(num_episodes):
        ...
        min_deviation = min(metrics['deviations']) if metrics['deviations'] else 0
        ...
        min_gimbal_smooth = min(metrics['gimbal_smoothness']) if metrics['gimbal_smoothness'] else 0
        ...
        min_gimbal_smoothness.append(min_gimbal_smooth)
        ...
        min_throttle_smooth = min(metrics['throttle_smoothness']) if metrics['throttle_smoothness'] else 0
        ...
        min_throttle_smoothness.append(min_throttle_smooth)
